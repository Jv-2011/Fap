...
socios = {
  ...
  3: ["Daniela", 15, "daniela123", 0, {
    '2022/10': ['25 12:56:14', 150000]
  }],
  4: ["Lina", 15, "lina123", 0, {
    '2022/10': ['25 12:56:14', 45000]
  }],
  5: ["Jaime", 15, "jaime123", 0, {
    '2022/10': ['25 12:56:14', 25000]
  }],
  6: ["Diana", 15, "diana123", 0, {
    '2022/10': ['25 12:56:14', 30000]
  }]
}
...
def cambioMes():
  ...
  if (
      month == 12
  ):  #si la fecha es el dia 31 del mes 12 la funcion volvera al dia 1ero (Enero)
    month = 1
    cambioYear()
  else:
    month += 1
...
def cuotasPrestamo(diccionario, usuario):
  ...
  fecha = str(year) + "/" + str(
    month
  )  #Traemos el año y el mes actual y los almacenamos en la variable fecha
  if (diccionario[usuario][3] == 0
      ):  #Primero, se verifica si el usuario tiene un préstamo asignado.
    borrarPantalla()
    ...
    if (fecha in diccionario[usuario][3]):  #Verifica si ya hay un pago
      ...
      cuotas = diccionario[usuario][3][
        1]  #Trae el numero de cuotas del prestamo
      prestamo = diccionario[usuario][3][0]  #Trae la cantidad del prestamo
      interes = diccionario[usuario][3][
        2]  #Trae el interes que tiene que ser aplicado a cada cuota
      print(
        "¿Desea pagar la cuota del mes?. La cuota de este mes es : $",
        str((prestamo / cuotas) + ((prestamo / cuotas) * interes))
      )  #Se muestra el monto de la cuota y se le pregunta al usuario si desea pagarla.
      cuotaMes = int(input("1.Si\n2.Salir\nSeleccione una opcion: "))
      if (cuotaMes == 1):  #Si la respuesta es afirmativa
        diccionario[usuario][3].append(
          fecha
        )  #Se agrega la fecha de pago al diccionario como un elemento más
        ahorrosTotales += (prestamo / cuotas) + (
          (prestamo / cuotas) * interes
        )  #Se actualizan los ahorros totales del fondo sumando el monto de la cuota
        prestamo -= (prestamo / cuotas
                     )  #El préstamo disminuye con el valor de la cuota
...
def prestamo():
  ...
  )  #Se pregunta al usuario el tipo de cliente que desea realizar el préstamo
  if (tipoCliente == 1):  # Si el cliente es un socio
    usuario = int(
      input("Digite la cedula del socio: "))  #Se le pregunta su cédula
    if (socios[usuario][3] == 0):  #Si el socio no tiene prestamos
      disponible = totalAhorradoSocio(
        usuario
      )  #Con esta información se calcula el total ahorrado por el socio para determinar cuanto dinero puede prestar
      disponible *= 0.9
    ...
    password = str(
      input("Digite la contraseña: ")
    )  #Se le piden unos datos para registrarlo en el diccionario de terceros
    print("Hay disponible para prestar: $", str(ahorrosTotales))
    while True:
      cantidadPrestamo = int(input("Digite la cantidad a prestar: "))
      cuotas = int(input("Digite la cantidad de cuotas: "))
      if (
          cantidadPrestamo <= ahorrosTotales
      ):  #Si la cantidad pedida para el prestamo es menor o igual al total que tiene el fondo
        terceros[cedula] = [
          nombre, edad, password,
          [cantidadPrestamo, cuotas, 0.02,
           str(year) + "/" + str(month)]
        ]  #Añade este prestamos a la posicion 3 del usario del diccionario tercero
        print("Usted ha hecho un prestamo por: $" + str(cantidadPrestamo))
        menuAdmin()
        break
...
def ahorroProgramado(cedula):
  ...
  for i in socios[cedula]:  #Recorre una lista de socios a partir de su cédula
    if isinstance(
        i, dict
    ):  #Para cada elemento de la lista, se verifica si es un diccionario (Si el usuario ya ha realizado algún ahorro en el mes actual)
      x = str(i.keys())
      x = x[11:-2].replace(
        "'", ""
      )  #Si lo es, se extrae el año y mes del diccionario y se compara con el año y mes pasados como parámetros
      if (
          x == (str(year) + "/" + str(month))
      ):  #Si coinciden(Si ya tiene un ahorro ese mes), se ejecuta la función menuSocios()
        menuSocios(cedula)
        break
...
def ahorro(cedula):
  ...
  for i in socios[cedula]:  #Recorre una lista de socios a partir de su cédula
    if isinstance(
      ...
      x = x[11:-2].replace(
        "'", ""
      )  #Si lo es, se extrae el año y mes del diccionario y se compara con el año y mes pasados
      if (
          x == (str(year) + "/" + str(month))
      ):  #Si es así, se agrega la hora actual y la cantidad ahorrada al diccionario correspondiente.
...
def loginTerceros():
  ...
  while True:
    ...
    passwordLogin = str(input("Digite su contraseña: "))
    if (
        passwordLogin == terceros[usuario][2]
    ):  #Este código comprueba si la contraseña ingresada por el usuario es igual a la contraseña almacenada en un diccionario llamado "terceros" para el usuario específico. Si las contraseñas coinciden, se ejecuta el código dentro del bloque if.
      print("Su ingreso ha sido autorizado, bienvenido")
      menuTerceros(usuario)
      break
    else:
...
def menuClientes():
  ...
  if (opcionClientes == 1):
    ...
    )  #En esta variable ejecutamos la funcion ahorroInicial para pedir y almacenar en esta misma un ahorro inicial que sea igual o mayor a 25000 en el formato de diccionario definido en la funcion para posteriormente almacenarlo en la posicion 4 del diccionario socios.
    socios[cedula] = [
      nombre, edad, password, 0, ahorro
    ]  #Almacenamos todos los datos pedidos para hacer el registro dentro del diccionario socios, siguiendo el orden que ya estaba definido.
  if (opcionClientes == 2):
    fap()
    tipoCliente = int(
      input("1.Socio\n2.Tercero\n3.Salir\nSeleccione una opcion: "))
